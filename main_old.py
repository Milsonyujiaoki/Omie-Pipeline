...
def carregar_configuracoes(config_path: str = CONFIG_PATH) -> Dict[str, Any]:
    """
    Carrega e valida as configuracões do sistema a partir do arquivo INI.
    
    Configuracões carregadas:
    - Diretorios de trabalho (paths)
    - Parâmetros de performance (batch_size, max_workers)
    - Modo de operacoo (async/parallel)
    - Credenciais da API Omie
    
    Args:
        config_path: Caminho para o arquivo de configuracoo INI
        
    Returns:
        Dict contendo todas as configuracões validadas do sistema
        
    Raises:
        SystemExit: Se arquivo INI noo existe ou configuracões obrigatorias ausentes
        ConfigParser.Error: Se arquivo INI malformado
        ValueError: Se valores numericos invalidos
    """
    config_file = Path(config_path)
    
    if not config_file.exists():
        logger.error(f"[CONFIG] Arquivo de configuracoo noo encontrado: {config_path}")
        logger.error(f"[CONFIG] Certifique-se de que o arquivo {config_path} existe no diretorio raiz")
        sys.exit(1)
    ...
    try:
        ...
        # Validacoo de secões obrigatorias
        required_sections = ['paths', 'pipeline', 'api_speed', 'omie_api', 'query_params']
        for section in required_sections:
            if section not in config:
                logger.error(f"[CONFIG] Secao obrigatoria [{section}] noo encontrada no arquivo INI")
                sys.exit(1)
        
        # Validacoo de chaves obrigatorias
        if 'resultado_dir' not in config['paths']:
            logger.error("[CONFIG] Chave obrigatoria 'resultado_dir' ausente na secao [paths]")
            sys.exit(1)
            
        # Validacoo de credenciais obrigatorias
        if 'app_key' not in config['omie_api'] or 'app_secret' not in config['omie_api']:
            logger.error("[CONFIG] Credenciais app_key e app_secret ausentes na secao [omie_api]")
            sys.exit(1)
        
        # Carregamento das configuracões com valores padroo seguros
        ...
        # Configurações de query/consulta
        start_date = config.get("query_params", "start_date", fallback="01/01/2025")
        end_date = config.get("query_params", "end_date", fallback="31/12/2025")
        records_per_page = int(config.get("query_params", "records_per_page", fallback="200"))
        
        # Validacoo de credenciais nao vazias
        if not app_key or not app_secret:
            logger.error("[CONFIG] Credenciais app_key e app_secret nao podem estar vazias")
            sys.exit(1)
        
        # Configuracões de performance com fallbacks inteligentes
        ...
        logger.info(f"[CONFIG] - Calls per second: {configuracoes['calls_per_second']}")
        logger.info(f"[CONFIG] - Records per page: {configuracoes['records_per_page']}")
        logger.info(f"[CONFIG] - Período: {start_date} a {end_date}")
        
        return configuracoes
        
    except (configparser.Error, ValueError) as e:
        logger.error(f"[CONFIG] Erro ao processar arquivo de configuracoo: {e}")
        sys.exit(1)
    except Exception as e:
        logger.error(f"[CONFIG] Erro inesperado ao carregar configuracões: {e}")
        sys.exit(1)
...
def executar_atualizacao_anomesdia() -> None:
    ...
    """
    try:
        logger.info("[PIPELINE.ANOMESDIA] Iniciando atualização da indexação temporal...")
        t0 = time.time()
        ...
        logger.info("[PIPELINE.ANOMESDIA] Processando registros sem indexação temporal...")
        ...
        try:
            with sqlite3.connect("omie.db") as conn:
                ...
                total_indexados = cursor.fetchone()[0]
                cursor.execute("SELECT COUNT(*) FROM notas")
                total_registros = cursor.fetchone()[0]
                
                if total_registros > 0:
                    percentual = (total_indexados / total_registros) * 100
                    logger.info(f"[PIPELINE.ANOMESDIA.COBERTURA] {total_indexados:,}/{total_registros:,} registros indexados ({percentual:.1f}%)")
                
        except Exception as stats_error:
            logger.debug(f"[PIPELINE.ANOMESDIA.STATS] Erro ao obter estatísticas: {stats_error}")
...
def criar_indices_performance(db_path: str) -> None:
    """
    Cria índices otimizados para consultas de detecção de modo e verificação.
    
    Índices criados:
    - idx_status_download: xml_baixado + erro (consulta principal)
    - idx_verificador_pendentes: xml_baixado = 0 (otimizado para verificador)
    - idx_erro_mensagem: erro + mensagem_erro (análise detalhada)
    - idx_data_emissao: dEmi (ordenação e filtros por data)
    - idx_anomesdia: anomesdia (consultas temporais otimizadas)
    - idx_anomesdia_*: índices compostos para filtros específicos
    
    Args:
        db_path: Caminho para o banco SQLite
    """
    indices_sql = [
        # Índice específico para verificador_xmls (alta performance)
        """CREATE INDEX IF NOT EXISTS idx_verificador_pendentes 
           ON notas(xml_baixado, cChaveNFe, dEmi, nNF) 
           WHERE xml_baixado = 0""",
        
        # Índice para consulta rápida de XMLs baixados
        ...
        # Índice para chave primária se não existir
        """CREATE UNIQUE INDEX IF NOT EXISTS idx_chave_nfe 
           ON notas(cChaveNFe)""",
        ...
        """CREATE INDEX IF NOT EXISTS idx_anomesdia_pendentes 
           ON notas(anomesdia, xml_baixado) 
           WHERE anomesdia IS NOT NULL AND xml_baixado = 0""",
        
        # Índice para verificação de XMLs vazios
    ...
    try:
        with conexao_otimizada(db_path) as conn:
            ...
            for sql in indices_sql:
                try:
                    conn.execute(sql)
                    nome_indice = sql.split()[5]  # Nome do índice
                    logger.debug(f"[ÍNDICE] Criado/Verificado: {nome_indice}")
                    indices_criados += 1
                except sqlite3.Error as e:
                    logger.warning(f"[ÍNDICE] Falha ao criar índice: {e}")
            
            conn.commit()
            logger.info(f"[ÍNDICE] {indices_criados}/{len(indices_sql)} índices de performance criados/verificados")
            
            # Análise de estatísticas do banco para otimização
            try:
                cursor = conn.cursor()
                cursor.execute("ANALYZE")
                logger.debug("[ÍNDICE] Estatísticas do banco atualizadas")
                
                # Criar view otimizada para verificador se não existir
                cursor.execute("""
                    ...
                    ORDER BY nIdNF
                """)
                logger.debug("[VIEW] View otimizada para verificador criada/verificada")
                
            except Exception as analyze_error:
                logger.debug(f"[ÍNDICE] Erro ao atualizar estatísticas/views: {analyze_error}")
            
    except Exception as e:
        logger.error(f"[ÍNDICE] Erro ao criar índices: {e}")


def _marcar_xml_com_erro(chave_nfe: str, erro: str) -> None:
    """Marca XML com erro no banco."""
    import sqlite3
    
    try:
        with sqlite3.connect("omie.db") as conn:
            cursor = conn.cursor()
            # Coluna erro_xml foi removida do schema
            pass
    except Exception as e:
        logger.error(f"[BANCO] Erro ao marcar XML com erro: {e}")
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.exception(f"[FASE 2.5] Erro durante otimização do banco: {e}")
            logger.warning("[FASE 2.5] Pipeline continuará sem otimizações completas")
            
        # =============================================================================
        # Fase 3: Indexação temporal otimizada
        # =============================================================================
        logger.info("[FASE 3] - Atualizando indexação temporal (anomesdia)...")
        try:
            executar_atualizacao_anomesdia()
            logger.info("[FASE 3] - ✓ Indexação temporal concluída com sucesso")
        except Exception as e:
            logger.exception(f"[FASE 3] Erro durante indexação temporal: {e}")
            logger.warning("[FASE 3] Pipeline continuará sem indexação temporal completa")

        
        # =============================================================================
        # Fase 3.5: Pipeline principal (download de XMLs)
        # =============================================================================
        logger.info("[FASE 3.5] - Executando pipeline principal...")

        try:
            # Atualiza datas de consulta antes da execução
            logger.info("[FASE 3.5] - Atualizando datas de consulta...")
            try:
                executar_atualizador_datas_query()
                logger.info("[FASE 3] - Datas de consulta atualizadas")
            except Exception as e:
                logger.error(f"[FASE 3] - Erro ao atualizar datas: {e}")
                logger.warning("[FASE 3] - Continuando com datas atuais")
